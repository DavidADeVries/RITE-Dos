...
UnitStr='Unit09';
ResolutionStr='Half';
ImAng=357   % You may write a script which finds this value in the RP Plan File.
MU=258.44     % You may write a script which finds this value in the RP Plan File.
...
% LOAD WED MATRICES (Water Equiv Depth)
%-------------------------------------------------------------------------
...
wed_CAX_corr=[mean(mean(wed_s_to_i(189:196,253:260))) mean(mean(wed_i_to_e(189:196,253:260))) mean(mean(wed_tot(189:196,253:260)))];
    ...
    all_figs(1)=figure; imagesc(wed_s_to_i); colorbar; axis equal; axis tight; title('WED, source to isoplane')
    all_figs(2)=figure; imagesc(wed_i_to_e); colorbar;  axis equal; axis tight; title('WED, isoplane to epid')
    all_figs(3)=figure; imagesc(wed_tot); colorbar;  axis equal; axis tight; title('WED, total')
...
EPID_folder=strcat(working_dir,'\Patient_EPID_images\GA',ImAngStr,'\fx',FxStr);

cd(working_dir)

cd Patient_plan_and_dose_maps
eval(['TPS_dosemap_name=''tpsGA',ImAngStr,'.dcm'';']) 
...
[epid,nCINEfiles,CAX,goodimages_numbers,badimages_numbers,example_cine]=EPID_prepare_4(EPID_folder);
cd(working_dir);
