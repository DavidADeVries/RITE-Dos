function [GAMMAvalue,figGamma] = GammaEval2(REF,MAP,DDtol,DTAtol,mask_or_ones)
%Stefano Peca & Brandon Koger, University of Calgary, 2013.
%This function does a gamma evaluation out of the two 2D input matrices.
%REF is the reference dose map (e.g. Dose map from TPS)
%MAP is the one to be evaluated (e.g. Measured dose map)
%(REF and MAP are usually DICOM files which you an import in MatLab with
%dicomread command)

MAP = double(MAP);
REF = double(REF);

%DDtol (dose difference tolerance) must be specified in percent, ex 3% is "3". 
DDtol2 = DDtol*max(REF(:))/100;

%DTAtol (distance to agreement tolerance) must be specified in mm, ex 3mm is "3", 
%NOTE: dose difference considered is a % of the MAX dose in the reference matrix.)
display ('Please verify the following, as it depends on the imager resolution:');
PixelSizeAtIso=267.6/512 %mm/pixels, make sure these 2 numbers are correct
display ('mm per pixel, projected at iso');
DTAtol2 = DTAtol / PixelSizeAtIso 
display ('in number of pixels')
% changes DTAtol from mm to number of pixels, 
% where pixelsize is the linear dimansion of 1 pixel at isocenter.
...
imagesc(GAMMAvalue.*mask_or_ones)
colorbar; set(gca, 'CLim', [0 2]); axis equal; axis tight;
str=sprintf('Gamma Evaluation, DD = %f percent and DTA = %f mm, masked',DDtol,DTAtol);
title(str)
