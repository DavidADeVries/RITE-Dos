...
static real_T e_emlrt_marshallIn(const emlrtStack *sp, const mxArray
  *sourceToAxisInM, const char_T *identifier);
  ...
  *sourceToAxisInM, const char_T *identifier)
{
  ...
  thisId.fParent = NULL;
  thisId.bParentIsCell = false;
  y = f_emlrt_marshallIn(sp, emlrtAlias(sourceToAxisInM), &thisId);
  emlrtDestroyArray(&sourceToAxisInM);
  return y;
}
...
void fastRayTrace_api(const mxArray * const prhs[8], const mxArray *plhs[2])
{
  ...
  real_T sourceToAxisInM;
  real_T axisToEpidInM;
  real_T waterEquivDoseSourceToIso;
  real_T waterEquivDoseIsoToEpid;
  ...
  sourceToAxisInM = e_emlrt_marshallIn(&st, emlrtAliasP(prhs[6]),
    "sourceToAxisInM");
  axisToEpidInM = e_emlrt_marshallIn(&st, emlrtAliasP(prhs[7]), "axisToEpidInM");

  /* Invoke the target function */
  fastRayTrace(&st, *pointSourceCoords, *pointDetectorCoords,
               *phantomLocationInM, *phantomDims, *voxelDimsInM, phantomData,
               sourceToAxisInM, axisToEpidInM, &waterEquivDoseSourceToIso,
               &waterEquivDoseIsoToEpid);

  /* Marshall function outputs */
  plhs[0] = emlrt_marshallOut(waterEquivDoseSourceToIso);
  plhs[1] = emlrt_marshallOut(waterEquivDoseIsoToEpid);
