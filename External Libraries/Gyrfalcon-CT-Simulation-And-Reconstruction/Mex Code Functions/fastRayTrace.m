function [waterEquivDoseSourceToIso, waterEquivDoseIsoToEpid] = fastRayTrace(pointSourceCoords, pointDetectorCoords, phantomLocationInM, phantomDims, voxelDimsInM, phantomData, sourceToAxisInM, axisToEpidInM)
...
if tMax < tMin
    waterEquivDoseSourceToIso = 0;
    waterEquivDoseIsoToEpid = 0;
    ...
    while endingT - currentT > Constants.Round_Off_Error_Bound
        ...
        relativeElectronDensity = phantomData(indices(2), indices(1), indices(3));
                
        currentT = nextT;
        currentPoint = nextPoint;
        
        if currentT <= tCrossover % in the source-to-iso area
            waterEquivDoseSourceToIso = waterEquivDoseSourceToIso + length .* relativeElectronDensity;
        else % in the iso-to-EPID area
            waterEquivDoseIsoToEpid = waterEquivDoseIsoToEpid + length .* relativeElectronDensity;
        end
