...
% -------------------------------------------------------------------------
% WHAT PATIENT?
working_dir='C:\Documents and Settings\stefanopeca\My Documents\EPID_dosimetry_RESEARCH\Pat00-U09';
cd(working_dir);
% WHAT BEAM?
ImAngStr='15X'; %Used to locate the EPID images directory and the correct reference TPS dose map
% WHAT FRACTION?
FxStr='04'; % note: this is not the actual fraction number (1...25), rather, it's the imaged fraction, or imaging session (1...9)
...
% % WHAT ENERGY?
EnergyStr='15X';
l=20; %Used for the correct big F=S/D factor, and TPR factor
% l_string and w_string are used to find the correct edge (off-axis) corrections
% (Used for empirical, convolution (gaussian weights), & backscatter (matrix) corrections)
l_string='20p0'; 
w_string='05p0'; 


collimator=0; 
nFractions=1; % used to normalize the TPS map to 1 fx

%eval(['TPS_dosemap_name=''tpsGA',ImAngStr,'.dcm'';']) % IF ACTUAL PATIENT
eval(['TPS_dosemap_name=''l',l_string,'w',w_string,'.dcm'';']); % IF COMISSIONING

% "FF_used" is the flood field image used to correct at moment of image
% acquisition. We want to remove this one
% "FF_new" is acquired seperately (before or after) through 20cm of water. We
% want to apply this one, to 'flatten' the EPID response after 20cm, to make it
% correspond more closely to the flat dose at half depth (10cm).
%FF_used='C:\Documents and Settings\stefanopeca\My Documents\EPID_dosimetry_RESEARCH\DF_FF_images\u9\20140610\MFF06729'
%FF_used='C:\Documents and Settings\stefanopeca\My Documents\EPID_dosimetry_RESEARCH\DF_FF_images\u10\20141103\MFF01957'
%FF_used='C:\Documents and Settings\stefanopeca\My Documents\EPID_dosimetry_RESEARCH\DF_FF_images\u9\20140716\MFF07263'
FF_used='C:\Documents and Settings\stefanopeca\My Documents\EPID_dosimetry_RESEARCH\DF_FF_images\u9\20140818\MFF07643'%U9 15X empty beam
%FF_used='C:\Documents and Settings\stefanopeca\My Documents\EPID_dosimetry_RESEARCH\DF_FF_images\u9\20140818\MFF07645'%U9 06X empty beam
FF_new='C:\Documents and Settings\stefanopeca\My Documents\EPID_dosimetry_RESEARCH\2014_10_21\MFF13409.dcm'%U9 06X, 20cm SW in beam
% -------------------------------------------------------------------------
% -------------------------------------------------------------------------

% LOAD WED MATRICES (CALCULATED PREVIOUSLY USING PROJECTION SCRIPT)
%load('C:\Documents and Settings\stefanopeca\My Documents\EPID_dosimetry_RESEARCH\WEDs\WED_20W_BAD.mat');% load WED matrices previously calculated from CT
cd(working_dir);

%load('WED-SW_w26_l6.mat')
...
wed_total_TRANSP=zeros(384,512)+w;

wed_source_to_iso_TRANSP=flipud(wed_source_to_iso_TRANSP); % verify if correct to flip
wed_iso_to_EPID_TRANSP=flipud(wed_iso_to_EPID_TRANSP);
wed_total_TRANSP=flipud(wed_total_TRANSP);

% CORRECT THE CALCULATED WEDS (OPTIONAL. BUT CHECK EVERY TIME!))
% The Ct data should reflect what is in the treatment room, including couch.
% For example, if you have 10cm of SW on the thin end of the IGRT couch, 
% then total WED along the CAX should be about 10.0+w(couch)=10.6 cm.
% NOTE this is an empirical correction which likely depends on the CT scanner,
% the couch you CT'd on, and on other factors.
wed_CAX_raw=[mean(mean(wed_source_to_iso_TRANSP(189:196,253:260))) ...
    mean(mean(wed_iso_to_EPID_TRANSP(189:196,253:260))) ...
    mean(mean(wed_total_TRANSP(189:196,253:260)))]

wed_s_to_i=wed_source_to_iso_TRANSP-0.0;
wed_i_to_e=wed_iso_to_EPID_TRANSP-0; %empirical, to account for the couch in the CT
wed_tot=wed_total_TRANSP-0;

% figure; imagesc(wed_s_to_i); colorbar; axis equal; axis tight;
% figure; imagesc(wed_i_to_e); colorbar;  axis equal; axis tight;
% figure; imagesc(wed_tot); colorbar;  axis equal; axis tight;
...
cd(working_dir); eval(['cd(''DOSE_from_Eclipse\',EnergyStr,' '')']);
...
% WHERE ARE THE EPID IMAGES? (should be automatic)
EPID_folder=strcat(working_dir,'\GA',ImAngStr,'\fx',FxStr)
[epid,flood_old,flood_new]=EPID_prepare_PATIENT(EPID_folder,FF_used,FF_new);
cd(working_dir);
...
% LOAD BACKSCATTER CORRECTION ON EPID IMAGE
eval(['load(''C:\Documents and Settings\stefanopeca\My Documents\EPID_dosimetry_RESEARCH\RITE Dos 2015\Backscatter_corr\',EnergyStr,'\bsc_l',l_string,'w',w_string,'.mat'');']);
...
% 6.2       MAKE f MATRIX 
fcol=round(2*l-9); %to get the correct column in f
...
DOSE_NOCORR=mask.*epid.*TMRratio.*fmatrix./Fmatrix;
...
w_cr_tot_avg=mean(w_cr_tot(143:242,:));
gsum_avg=w_cr_tot_avg(1)*g1+w_cr_tot_avg(2)*g2+w_cr_tot_avg(3)*g3+w_cr_tot_avg(4)*g4;
...
%   GIVE THEM RELATIVE WEIGHTS
    ...
    conv_1(row,:)=conv(profile,gsum_avg,'same');
...
w_in_tot_avg=mean(w_in_tot(:,207:306));
%gsum_avg=w_in_tot_avg(1)*g1+w_in_tot_avg(2)*g2+w_in_tot_avg(3)*g3+w_in_tot_avg(4)*g4;
...
%   GIVE THEM RELATIVE WEIGHTS
    ...
    conv_2(:,col)=conv(profile,gsum_avg,'same');
...
DOSE_CONV=mask.*conv_2.*TMRratio.*fmatrix./Fmatrix;
...
eval(['load(''C:\Documents and Settings\stefanopeca\My Documents\EPID_dosimetry_RESEARCH\RITE Dos 2015\empirical_corr\cc_l',l_string,'w',w_string,'.mat'');']);
% If you get an error saying there is no such file, make it using comissioning
% square fields and the script
...
DOSE_EMPIRICAL=mask.*epid.*TMRratio.*fmatrix./Fmatrix./(F_edge_corrTot+1e-10);
% (the "+1e-10" is to prevent 0/0=NaN)



% 14.b        CORRECT DOSE CONV with pre-calculated correction MATRIX
%eval(['load(''C:\Documents and Settings\stefanopeca\My Documents\EPID_dosimetry_RESEARCH\RITE Dos 2015\Backscatter_corr\bsc_l',l_string,'w',w_string,'.mat'');']);
eval(['load(''C:\Documents and Settings\stefanopeca\My Documents\EPID_dosimetry_RESEARCH\RITE Dos 2015\Backscatter_corr\bsc_l20p0w',w_string,'.mat'');']);
...
% 16.1       CROSS-PLANE
figure; 
plot(1:512,eclipse(192,:),1:512,DOSE_NOCORR(192,:),1:512,DOSE_EMPIRICAL(192,:),1:512,DOSE_CONV(192,:),1:512,DOSE_CONV_bsc(192,:));
legend('TPS','F(CAX) only','Empirical corr','Convolution corr','Convolution+BSC corr','location','South'); title('Cross-plane dose');xlim([1 512]);ylim([0 DOSEmax]);
% 16.2       IN-PLANE
figure; 
plot((1:384)+64,eclipse(:,256),(1:384)+64,DOSE_NOCORR(:,256),(1:384)+64,DOSE_EMPIRICAL(:,256),(1:384)+64,DOSE_CONV(:,256),(1:384)+64,DOSE_CONV_bsc(:,256));
legend('TPS','F(CAX) only','Empirical corr','Convolution corr','Convolution+BSC corr','location','South'); title('In-plane dose');xlim([1 512]);ylim([0 DOSEmax]);
...
axis equal; axis tight; colorbar; set(gca, 'CLim', [-10 10]);
...
clear c d;
figure; imagesc(PercentDiffMatrix_Conv_bsc); title 'Percent difference, Convolution & Backscatter correction'
axis equal; axis tight; colorbar; set(gca, 'CLim', [-10 10]);
...
% 
% eval(['save dosemaps_GA',ImAngStr,'_fx',FxStr,' DOSE_NOCORR_fx',FxStr,' DOSE_EMPIRICAL_fx',FxStr,' DOSE_CONV_fx',FxStr,' epid_fx',FxStr,' mask_difference_fx',FxStr,' eclipse;']);
% 
