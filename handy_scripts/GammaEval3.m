function [GAMMAvalue,figGamma] = GammaEval2(REF,MAP,DDtol,DTAtol)
...
%MAP is the one to be evaluated (e.g. Measured or other)
...
%REF and MAP have to be 2D arrays (matrices)
%If your starting REF and MAP are dicom files, then you can import them in 
%MatLab with the commands: dicomread and dicominfo
%For example, in Varian Eclipse I do:
    % REF=dicomread(YourDoseFromTPS.dcm); 
    % REF_HEADER=dicominfo(YourDoseFromTPS.dcm); 
    % REF=double(REF);
    % REF=100*REF*REF_HEADER.DoseGridScaling;
%This may change in different TPSs.
% ------------------------------------------------------------------------
% First, make sure maps are double precision
MAP = double(MAP);
REF = double(REF);
...
PixelSizeAtIso=267.6/512 %mm/pixels. 267/512 is true for Varian aSi half resolution. You may have to change this.
display ('mm per pixel, projected at iso');
% ------------------------------------------------------------------------

DTAtol2 = DTAtol / PixelSizeAtIso; % This is DTA in number of pixels rather than mm
...
str=sprintf('Gamma Evaluation: DD = %f percent, DTA = %f mm, masked',DDtol,DTAtol);
title(str)
