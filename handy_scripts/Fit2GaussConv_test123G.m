function [estimates,ConvRescaleFactor,ErrorVector,SumSquares,w_num_pts] = Fit2GaussConv(ydata, ref, SD1, SD2, SD3, SD4)
% Call fminsearch with a random starting point.
start_point = rand(1,3);
model = @ConvSumGauss;
options = optimset('MaxFunEvals',10000,'MaxIter',10000);
%estimates = fminsearchbnd(model, start_point, [0.01 0.01 0.01 0.01], [],options);
estimates = fminsearchbnd(model, start_point, [0.01 0.01 0.01 ], [],options);
...
%        gsum=(w1*g1+w2*g2+w3*g3+w4*g4)/trapz(w1*g1+w2*g2+w3*g3+w4*g4);
                gsum=(w1*g1+w2*g2+w3*g3)/trapz(w1*g1+w2*g2+w3*g3);
        temp=ref;
