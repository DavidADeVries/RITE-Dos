function [ w_cr_avg, w_in_avg, ConvRescaleFactor_cross, ConvRescaleFactor_in, HCM ] = makeGaussianCorr( ECLIPSEs, BSCEPIDsF )
...
HCM = zeros(size(BSCEPIDsF));

for i=1:size(BSCEPIDsF,3)
    ...
    epid_max = mean2(BSCEPIDsF(189:196,253:260,i));
    epid_min = mean2(BSCEPIDsF(1:8,1:8,i));
    mask = (BSCEPIDsF(:,:,i) > abs(epid_max+epid_min)/4);
    ...
    shiftEPIDsF = circshift(BSCEPIDsF(:,:,i), [round(epidsagSIsign*epidsagSImagn) round(epidsagLRsign*epidsagLRmagn)]);
    assignin('base','shiftEPIDsF',shiftEPIDsF)
...
%     xlim([b-30 d+30]); ylim([0.8*max(tps_profile) 1.1*max(tps_profile)])
    ...
    % Then do HCM(:,:,i) = makeGHC(shiftBSCEPIDsF);
end
